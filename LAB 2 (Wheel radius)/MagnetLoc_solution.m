...
X = [ 0, 0, 0*pi/180, rwheel+2.0, rwheel-1.0 ].' ;  
...
for i = 2 : nbLoops 
    ...
    A = [ 1 , 0 , -deltaD*sin(X(3)) ,  U(1)/2*cos(X(3)) ,  U(2)/2*cos(X(3)) ;
          0 , 1 ,  deltaD*cos(X(3)) ,  U(1)/2*sin(X(3)) ,  U(2)/2*sin(X(3)) ;
          0 , 0 ,        1          ,  U(1)/trackGauge  , -U(2)/trackGauge  ;
          0 , 0 ,        0          ,         1         ,          0        ;
          0 , 0 ,        0          ,         0         ,          1        ] ;
    B = [ 1/2*X(4)*cos(X(3)) ,  1/2*X(5)*cos(X(3)) ;
          1/2*X(4)*sin(X(3)) ,  1/2*X(5)*sin(X(3)) ; 
    ...
    for measNumber = 1 : length(measures) 
        ...
        mRealMagnet = mTo * oRealMagnet ;  
        
        % The expected measurement are the two coordinates of the real 
        % magnet in the robot frame.
        Yhat = mRealMagnet(1:2) ;
        
        C = [ -cos(X(3)) , -sin(X(3)) , -sin(X(3))*(oRealMagnet(1)-X(1))+cos(X(3))*(oRealMagnet(2)-X(2)) , 0 , 0 ;
               sin(X(3)) , -cos(X(3)) , -sin(X(3))*(oRealMagnet(2)-X(2))-cos(X(3))*(oRealMagnet(1)-X(1)) , 0 , 0 ] ;
